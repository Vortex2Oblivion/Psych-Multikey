...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		for (i in 0...(ManiaData.keyCounts[PlayState.SONG.keyCount] * 2)){
			var note:StrumNote = new StrumNote(GRID_SIZE * (i+1), strumLine.y, i % ManiaData.keyCounts[PlayState.SONG.keyCount], 0);
	...
	function addSongUI():Void
	{
		...
		tab_group_song.add(new FlxText(stepperSpeed.x, stepperSpeed.y + 150, 0, 'Key Count:'));
		...
		keyNumber = new FlxUINumericStepper(stepperSpeed.x + 100, stepperSpeed.y + 150, 1, 4, 1, 9, 0);
		if (keyNumber.value > 9)
			keyNumber.value = 9
		else
			keyNumber.value = PlayState.SONG.keyCount + 1;

		if (keyNumber.value < 1)
			keyNumber.value = 4
		else
			keyNumber.value = PlayState.SONG.keyCount + 1;
		keyNumber.name = 'key_amount';
		blockPressWhileTypingOnStepper.push(keyNumber);

		tab_group_song.add(keyNumber);
	...
	function addSectionUI():Void
	{
		...
		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "Swap section", function()
		{
			...
			{
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
				note[1] = (note[1] + ManiaData.keyCounts[PlayState.SONG.keyCount]) % (ManiaData.keyCounts[PlayState.SONG.keyCount] * 2);
		...
		var duetButton:FlxButton = new FlxButton(10, copyLastButton.y + 45, "Duet Notes", function()
		{
			...
			{
				var boob = note[1];
				if (boob > PlayState.SONG.keyCount){
					boob -= ManiaData.keyCounts[PlayState.SONG.keyCount];
				}else{
					boob += ManiaData.keyCounts[PlayState.SONG.keyCount];
				}
		...
		var mirrorButton:FlxButton = new FlxButton(duetButton.x + 100, duetButton.y, "Mirror Notes", function()
		{
			...
			for (note in _song.notes[curSec].sectionNotes)
			{
				var boob = note[1]% ManiaData.keyCounts[PlayState.SONG.keyCount];
				boob = PlayState.SONG.keyCount - boob;
				if (note[1] > PlayState.SONG.keyCount) boob += ManiaData.keyCounts[PlayState.SONG.keyCount];
	...
	var keyNumber:FlxUINumericStepper;
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			switch(wname)
			{
				...
				case 'key_amount':
					PlayState.SONG.keyCount = Std.int(nums.value) - 1;
					reloadGridLayer();
			}
		}
	...
	override function update(elapsed:Float)
	{
		...
		var gWidth = GRID_SIZE * (ManiaData.keyCounts[PlayState.SONG.keyCount] * 2);
		camPos.x = -80 + gWidth;
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			...
			if(curSelectedNote != null) {
				...
				if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += ManiaData.keyCounts[PlayState.SONG.keyCount];
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					var noteDataToCheck:Int = note.noteData % ManiaData.keyCounts[PlayState.SONG.keyCount];
					if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += ManiaData.keyCounts[PlayState.SONG.keyCount];
	...
	*/
	...
	function reloadGridLayer() {
		GRID_SIZE = ManiaData.gridScales[PlayState.SONG.keyCount];

		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * ManiaData.keyCounts[PlayState.SONG.keyCount] * 2, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));
		...
		var oneHalf:Float = GRID_SIZE * ManiaData.keyCounts[PlayState.SONG.keyCount];
		...
		if(sectionStartTime(1) <= FlxG.sound.music.length)
		{
			...
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * ManiaData.keyCounts[PlayState.SONG.keyCount] * 2, leHeight + Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
		...
		if(foundNextSec)
		{
			var gridBlack:FlxSprite = new FlxSprite(0, gridBG.height).makeGraphic(Std.int(GRID_SIZE + GRID_SIZE * ManiaData.keyCounts[PlayState.SONG.keyCount] * 2), Std.int(nextGridBG.height), FlxColor.BLACK);
		...
		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * ManiaData.keyCounts[PlayState.SONG.keyCount])).makeGraphic(2, leHeight, FlxColor.BLACK);
	...
	function updateWaveform() {
		...
		if(waveformPrinted) {
			...
			if(lastWaveformHeight != height && waveformSprite.pixels != null)
			{
				...
				waveformSprite.makeGraphic(Std.int(GRID_SIZE * (ManiaData.keyCounts[PlayState.SONG.keyCount] * 2)), Std.int(gridBG.height), 0x00FFFFFF);
		...
		// Draws
		var gSize:Int = Std.int(GRID_SIZE * (ManiaData.keyCounts[PlayState.SONG.keyCount] * 2));
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[1] > ManiaData.keyCounts[PlayState.SONG.keyCount] - 1) note.mustPress = !note.mustPress;
		}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var note:Note = new Note(daStrumTime, daNoteInfo % ManiaData.keyCounts[PlayState.SONG.keyCount], null, null, true);
		...
		if(isNextSection && _song.notes[curSec].mustHitSection != _song.notes[curSec+1].mustHitSection) {
			if(daNoteInfo > ManiaData.keyCounts[PlayState.SONG.keyCount] - 1) {
				note.x -= GRID_SIZE * ManiaData.keyCounts[PlayState.SONG.keyCount];
			} else if(daSus != null) {
				note.x += GRID_SIZE * ManiaData.keyCounts[PlayState.SONG.keyCount];
			}
		}
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if(note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += ManiaData.keyCounts[PlayState.SONG.keyCount];
			for (i in _song.notes[curSec].sectionNotes)
			{
	...
	public function doANoteThing(cs, d, style){
		...
		if(strumLineNotes.members[d].overlaps(curRenderedNotes))
		{
			curRenderedNotes.forEachAlive(function(note:Note)
			{
				if (note.overlapsPoint(new FlxPoint(strumLineNotes.members[d].x + 1,strumLine.y+1)) && note.noteData == d%ManiaData.keyCounts[PlayState.SONG.keyCount])
				{
						//trace('tryin to delete note...');
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		if(noteData > -1)
		{
			_song.notes[curSec].sectionNotes.push([noteStrum, noteData % (ManiaData.keyCounts[PlayState.SONG.keyCount] * 2), noteSus, curNoteTypes[daType]]);
		...
		{
			_song.notes[curSec].sectionNotes.push([noteStrum, (noteData + ManiaData.keyCounts[PlayState.SONG.keyCount]) % (ManiaData.keyCounts[PlayState.SONG.keyCount] * 2), noteSus, curNoteTypes[daType]]);
		}
