...
class EditorPlayState extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float)
	{
		...
		if(notes.length > 0)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				var strum:StrumNote = strumGroup.members[daNote.noteData % ManiaData.keyCounts[PlayState.SONG.keyCount]];
	...
	function generateSong(dataPath:String)
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				var daNoteData:Int = Std.int(songNotes[1] % ManiaData.keyCounts[PlayState.SONG.keyCount]);
				var gottaHitNote:Bool = section.mustHitSection;

				if (songNotes[1] > ManiaData.keyCounts[PlayState.SONG.keyCount] - 1)
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...ManiaData.keyCounts[PlayState.SONG.keyCount])
		{
			// FlxG.log.add(i);
	...
	private function getKeyFromEvent(key:FlxKey):Int
	{
		if(key != NONE)
		{
			for (i in 0...keysArray[PlayState.SONG.keyCount].length)
			{
				for (j in 0...keysArray[PlayState.SONG.keyCount][i].length)
				{
					if(key == keysArray[PlayState.SONG.keyCount][i][j])
					{
						return i;
					}
				}
	...
	private function dataKeyIsPressed(data:Int):Bool
		{
			for (i in 0...keysArray[PlayState.SONG.keyCount][data].length) {
				if (FlxG.keys.checkStatus(keysArray[PlayState.SONG.keyCount][data][i], PRESSED)) return true;
			}
	...
	private function keysCheck():Void
		{
			...
			for (key in keysArray)
				holdArray.push(controls.pressed(key));
					...
					{
						// hold note functions
						if ( daNote.isSustainNote && dataKeyIsPressed(daNote.noteData % ManiaData.keyCounts[PlayState.SONG.keyCount]) && daNote.canBeHit
