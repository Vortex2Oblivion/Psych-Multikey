...
class PlayState extends MusicBeatState
{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				var daNoteData:Int = Std.int(songNotes[1] % ManiaData.keyCounts[SONG.keyCount]);
				var gottaHitNote:Bool = section.mustHitSection;

				if (songNotes[1] > ManiaData.keyCounts[SONG.keyCount] - 1)
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...ManiaData.keyCounts[SONG.keyCount])
		{
			...
			babyArrow.animation.addByPrefix('static', ManiaData.staticAnimations[PlayState.SONG.keyCount][i] + ' static', 24, false);
			babyArrow.animation.addByPrefix('pressed', ManiaData.noteAnimations[PlayState.SONG.keyCount][i] + ' press', 24, false);
			babyArrow.animation.addByPrefix('confirm', ManiaData.noteAnimations[PlayState.SONG.keyCount][i] + ' confirm', 24, false);
				...
				if(notes.length > 0)
				{
					if(startedCountdown)
					{
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							var strum:StrumNote = strumGroup.members[daNote.noteData % ManiaData.keyCounts[SONG.keyCount]];
	...
	private function getKeyFromEvent(key:FlxKey):Int
		{
			if(key != NONE)
				{
					for (i in 0...keysArray[SONG.keyCount].length)
					{
						for (j in 0...keysArray[SONG.keyCount][i].length)
						{
							if(key == keysArray[SONG.keyCount][i][j])
							{
								return i;
							}
						}
	...
	{
		for (i in 0...keysArray[SONG.keyCount][data].length) {
			if (FlxG.keys.checkStatus(keysArray[SONG.keyCount][data][i], PRESSED)) return true;
		}
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
			{
				...
				{
					// hold note functions
					if (strumsBlocked[daNote.noteData] != true && daNote.isSustainNote && dataKeyIsPressed(daNote.noteData % ManiaData.keyCounts[SONG.keyCount]) && daNote.canBeHit
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if(char != null && char.hasMissAnimations)
		{
			...
			var animToPlay:String = "sing" + ManiaData.playerAnimations.get(ManiaData.noteAnimations[SONG.keyCount][direction]).toUpperCase() + "miss";
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			var char:Character = dad;
			var animToPlay:String = "sing" + ManiaData.playerAnimations.get(ManiaData.noteAnimations[SONG.keyCount][note.noteData]).toUpperCase() + altAnim;
			if(note.gfNote) {
				char = gf;
			}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				var animToPlay:String = "sing" + ManiaData.playerAnimations.get(ManiaData.noteAnimations[SONG.keyCount][note.noteData]).toUpperCase();
