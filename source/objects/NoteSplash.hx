...
class NoteSplash extends FlxSprite
{
	...
	public function setupNoteSplash(x:Float, y:Float, direction:Int = 0, ?note:Note = null) {
		...
		setGraphicSize(Std.int(width * ManiaData.splashSizes[PlayState.SONG.keyCount]));
		aliveTime = 0;
	...
	function loadAnims(skin:String, ?animName:String = null):NoteSplashConfig {
		...
		while(true) {
			...
			for (i in 0...ManiaData.keyCounts[PlayState.SONG.keyCount]) {
				if (!addAnimAndCheck('note$i-$animID', '$animName ${ManiaData.noteAnimations[PlayState.SONG.keyCount][i]} $animID', 24, false)) {
